...
EMBEDDING_OUTPUT_DIR=/data/group_data/cx_group/query_generation_data/temporary_indexes/$base_model
HF_DATASET_OUTPATH=/data/user_data/jmcoelho/datasets/11797/marco_passage/self_hard_negs/$base_model
mkdir -p $EMBEDDING_OUTPUT_DIR
# Use base models for self-hard negatives
# for shard in {0..7}; do
#     (
...
#     --dataset_split train \
#     --encode_output_path $EMBEDDING_OUTPUT_DIR/query-test-marco-passage-train.pkl

# set -f && OMP_NUM_THREADS=24 python -m tevatron.retriever.driver.search \
#     --query_reps $EMBEDDING_OUTPUT_DIR/query-test-marco-passage-train.pkl \
#     --passage_reps $EMBEDDING_OUTPUT_DIR/corpus.marco-passage.*.pkl \
#     --depth 100 \
#     --batch_size 128 \
#     --save_text \
#     --save_ranking_to $EMBEDDING_OUTPUT_DIR/run.marco-passage-train.txt

NEGS_DIR=$EMBEDDING_OUTPUT_DIR/run.marco-passage-train.txt
#python tevatron/scripts/build_train_dataset.py $NEGS_DIR $HF_DATASET_OUTPATH

model_output_name=$base_model-marco-passage-self-negativesep-0
...
#   --model_name_or_path "/data/user_data/jmcoelho/models/$base_model" \
#   --dataset_name $HF_DATASET_OUTPATH \
#   --save_steps 20000000 \
#   --query_prefix "Query: " \
...
# python -m pyserini.eval.trec_eval -c -mndcg_cut.10 -mrecip_rank msmarco-passage-dev-subset $EMBEDDING_OUTPUT_DIR/run.marco-passage-dev.trec

eval "$(conda shell.bash hook)"
conda activate qa_proj

HF_DATASET_OUTPATH=/data/user_data/jmcoelho/datasets/11797/marco_passage/self_hard_negs/$model_output_name
...
#python tevatron/scripts/build_train_dataset_momentum.py $EMBEDDING_OUTPUT_DIR/run.marco-passage-train.txt $NEGS_DIR $HF_DATASET_OUTPATH


ep2_model_output_name=$base_model-marco-passage-self-negativesep-1
EMBEDDING_OUTPUT_DIR=/data/group_data/cx_group/query_generation_data/temporary_indexes/$ep2_model_output_name
mkdir -p $EMBEDDING_OUTPUT_DIR
...
deepspeed --include localhost:0,1,2,3,4,5,6,7 --master_port $port --module tevatron.retriever.driver.train \
  ...
  --dataset_cache_dir $HF_HOME \
  --cache_dir $HF_HOME \
  --output_dir /data/user_data/jmcoelho/models/$ep2_model_output_name \
  --model_name_or_path "/data/user_data/jmcoelho/models/$base_model" \
  --dataset_name $HF_DATASET_OUTPATH \
  --save_steps 20000000 \
  --query_prefix "Query: " \
  ...
  --gradient_accumulation_steps 2 \
  --report_to wandb \
  --run_name $ep2_model_output_name
...
for shard in {0..7}; do
    ...
    if [ -f "$EMBEDDING_OUTPUT_FILE" ]; then
        ...
        CUDA_VISIBLE_DEVICES=$shard python -m tevatron.retriever.driver.encode \
                --output_dir=temp \
                --bf16 \
                --model_name_or_path /data/user_data/jmcoelho/models/$ep2_model_output_name \
                --dataset_cache_dir $HF_HOME \
                --cache_dir $HF_HOME \
                --query_prefix "Query: " \
                --passage_prefix "Passage: " \
