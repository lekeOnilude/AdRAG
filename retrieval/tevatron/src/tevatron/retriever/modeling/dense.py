...
class DenseModel(EncoderModel):
    ...
    def _pooling(self, last_hidden_state, attention_mask):
        ...
        elif self.pooling in ['wavg']:
            # scale hidden states so that later ones are more influential
            attention_mask_ = attention_mask * attention_mask.cumsum(dim=1)
            s = last_hidden_state * attention_mask_.unsqueeze(-1).float()
            d = attention_mask_.sum(dim=1, keepdim=True).unsqueeze(1).float() / attention_mask.sum(dim=1, keepdim=True).unsqueeze(1).float() 
            scaled_states = s / d

            masked_hiddens = scaled_states.masked_fill(~attention_mask[..., None].bool(), 0.0)
